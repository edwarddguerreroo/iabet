...
import numpy as np
from models.tft.base.config import TFTConfig #Importar configuración
import json
...
class TFT(Model):
    def __init__(self, config: Union[str, Dict, TFTConfig] = "config/tft_config.yaml", **kwargs):
        ...
        if sum([self.config.use_reformer_attention,
                self.config.use_multi_query_attention]) > 1:  # Modificado para usar la suma
    ...
    def call(self, inputs: Tuple[tf.Tensor, ...], training=None) -> tf.Tensor:
        ...
        current_index = 0
        time_varying_numeric_inputs = inputs[current_index]
        current_index += 1

        time_varying_categorical_inputs = []
        for _ in range(len(self.config.time_varying_categorical_features_cardinalities)):
            time_varying_categorical_inputs.append(inputs[current_index])
            current_index += 1

        static_numeric_inputs = inputs[current_index]
        current_index += 1

        static_categorical_inputs = []
        for _ in range(len(self.config.static_categorical_features_cardinalities)):
            static_categorical_inputs.append(inputs[current_index])
            current_index += 1

        time_inputs = inputs[current_index]
        current_index += 1

        gnn_input = None
        if self.config.use_gnn:
            gnn_input = inputs[current_index]
            current_index += 1

        transformer_input = None
        if self.config.use_transformer:
            transformer_input = inputs[current_index]
            # current_index no se incrementa aquí porque es el último elemento
    ...
    def load(self, filepath: str):
        ...
        dummy_inputs = (
            tf.zeros((1, self.config.seq_len, self.config.raw_time_features_dim)),  # (batch_size, seq_len, raw_time_features)
            [tf.zeros((1, self.config.seq_len, 1), dtype=tf.int32) for _ in range(len(self.config.time_varying_categorical_features_cardinalities))],  # (batch_size, seq_len, num_cat_time_features)
            tf.zeros((1, self.config.raw_static_features_dim)),  # (batch_size, raw_static_features)
            [tf.zeros((1, 1),  dtype=tf.int32) for _ in range(len(self.config.static_categorical_features_cardinalities))],  # (batch_size, num_cat_static_features)
            tf.zeros((1, self.config.seq_len, 1)) #Time input
        )
